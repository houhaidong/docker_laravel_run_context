		...
		$(selector).click(function()
		{
			...
			$('.aw-topic-bar .add').click(function()
			{
				switch (data_type)
				{
					...
					case 'question' :
						var _this = $(this);
						$.post(G_BASE_URL + '/tag/ajax/save_topic_relation/', 'type=question&item_id=' + data_id + '&topic_title=' + encodeURIComponent($(this).parents('.aw-topic-bar').find('.topic-text').val()), function(result)
						{
							if (result.errno == 1)
							{
					...
					case 'article' :
						var _this = $(this);
						$.post(G_BASE_URL + '/tag/ajax/save_topic_relation/', 'type=article&item_id=' + data_id + '&topic_title=' + encodeURIComponent($(this).parents('.aw-topic-bar').find('.topic-text').val()), function (result)
						{
							if (result.errno == 1)
							{
								_this.parents('.aw-topic-bar').find('.tag-bar').prepend('<span class="topic-tag" data-id="'+ result.rsm.topic_id +'"><a class="text">' + _this.parents('.aw-topic-bar').find('.topic-text').val() + '</a><a class="close"><i class="icon icon-delete"></i></a></span>');
